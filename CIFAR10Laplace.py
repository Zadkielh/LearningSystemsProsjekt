...
if __name__ == "__main__":
    ...
    for i in range(X_train.shape[0]):
        for j in range(X_train.shape[3]):
            X_train[i, :, :, j] = cv2.Laplacian(X_train[i, :, :, j], cv2.CV_64F)

    for i in range(X_test.shape[0]):
        for j in range(X_test.shape[3]):
            X_test[i, :, :, j] = cv2.Laplacian(X_test[i, :, :, j], cv2.CV_64F)
    ...
    for epoch in range(args.epochs):
        ...
        np.savetxt("CIFAR10Laplace_%d_%d_%d_%.1f_%d_%d_%d.txt" % (epoch+1, args.num_clauses, args.T, args.s, args.patch_size, args.max_included_literals, args.weighted_clauses), Y_test_scores, delimiter=',')